...
export const InternationalTripPage = () => {
    ...
    useEffect(() => {
        const fetchTrips = async () => {
            ...
            try {
                ...
                const target = displayCountry.toUpperCase();
                const filtered = data.filter(
                    (t) => t.pais && String(t.pais).toUpperCase() === target
                );

                filtered.sort((a, b) => {
                    const mA = a.mes_inicio ? String(a.mes_inicio).toUpperCase() : '';
                    const mB = b.mes_inicio ? String(b.mes_inicio).toUpperCase() : '';
                    return (MONTH_ORDER[mA] || 0) - (MONTH_ORDER[mB] || 0);
                });

                const g = filtered.reduce((acc, trip) => {
                    const m = trip.mes_inicio ? String(trip.mes_inicio).toUpperCase() : 'SIN MES';
                    if (!acc[m]) acc[m] = [];
                    acc[m].push(trip);
                    return acc;
                }, {});
    ...
    const months = useMemo(() => Object.keys(grouped), [grouped]);
    ...
    return (
        <div className="tv-page">
            ...
            <header className="tv-hero">
                ...
                <div className="tv-hero-inner">
                    <p className="tv-breadcrumb">
                        <Link to="/">Inicio</Link>
                        <span>/</span> Destinos
                        <span>/</span> <strong>{displayCountry}</strong>
                    </p>
                    <span className="tv-hero-tag">EXPEDICIÓN INTERNACIONAL · 2026</span>
                    <h1 className="tv-hero-title">
                        <span>DESCUBRE</span>
                        <span className="tv-hero-accent">{displayCountry.toUpperCase()}.</span>
                    </h1>
                    <p className="tv-hero-sub">
                        El mundo es enorme y tú apenas estás empezando.
                        {totalTrips > 0 && ` ${totalTrips} expediciones te esperan.`}
                    </p>
                </div>
            ...
            {!error && months.length === 0 && (
                <div className="tv-state">
                    <p>No tenemos viajes a {displayCountry} con cupos disponibles por ahora.</p>
                    <Link to="/todos-los-viajes" className="tv-reset">VER TODOS LOS DESTINOS</Link>
                </div>
            )}

            {/* CONTENIDO POR MES */}
            {!error && months.map((month, mi) => (
                <section key={month} className="col-month">
                    <div className="col-month-head">
                        <span className="col-month-num">{String(mi + 1).padStart(2, '0')}</span>
                        <h2 className="col-month-name">{month}</h2>
                        <span className="col-month-line" />
                        <span className="col-month-count">
                            {grouped[month].length} {grouped[month].length === 1 ? 'viaje' : 'viajes'}
                        </span>
                    </div>

                    <div className="tv-grid">
                        {grouped[month].map((trip, i) => (
            ...
            <section className="tv-cta">
                <h2>¿OTRO DESTINO<br />EN LA MIRA?</h2>
                <p>Explora todas nuestras travesías por el mundo.</p>
